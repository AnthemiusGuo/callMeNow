...
<div class="row">
    ...
    <div class="col-lg-12">
        <ul id="nav-store" class="nav nav-tabs">
            <?php
            foreach ($this->sub_menus as $key => $value) :
            ?>
                <li id="nav-store-<?php echo $key ?>" class="<?=($this->now_sub_menu==$key)?'active':'';?>"><a href="#" onclick="info_load('store','<?php echo $key ?>')"><?php echo $value['name'] ?></a></li>
            <?php
            endforeach;
            ?>
        </ul>
...
<div id="store_info">
    ...
    foreach ($this->sub_menus as $key => $value) :
        ...
        echo '<div class="info-store row '.$is_hidden.'" id="info-store-'.$key.'"><div class="col-md-12">';
        include_once("info-store-".$key.".php");
        echo '</div></div>';
    endforeach;
    ?>
</div>
